...
Node<Key, Data, Extra> * FindByRank(int k) {
  ...
  */
  Key getPredecessor(Key key){
    Key predecessorKey;
    getPredecessor_aux(key, root, &predecessorKey);
...
void printInOrder_aux(Node<Key, Data, Extra> * router)
{
  ...
  void getPredecessor_aux(Key key, Node<Key, Data, Extra>* curr, Key *predecessor){
    if(!curr){
      return;
    }
    if(curr->key >= key){ //going left
      getPredecessor_aux(key, curr->left, predecessor);
    } else { //going right
      *predecessor =  curr->key;
      getPredecessor_aux(key, curr->right, predecessor);
    }
  }
